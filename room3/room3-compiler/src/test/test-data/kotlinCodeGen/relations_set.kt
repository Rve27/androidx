...
internal class MyDao_Impl(
  ...
  public override fun getArtistAndSongs(): ArtistAndSongs {
    ...
    return performBlocking(__db, true, false) { _connection ->
      ...
      try {
        ...
        val _collectionSongs: MutableMap<Long, MutableSet<Song>> = mutableMapOf()
        while (_stmt.step()) {
          val _tmpKey: Long
          _tmpKey = _stmt.getLong(_columnIndexOfArtistId)
          if (!_collectionSongs.containsKey(_tmpKey)) {
            _collectionSongs.put(_tmpKey, mutableSetOf())
          }
        }
        _stmt.reset()
        ...
        if (_stmt.step()) {
          ...
          val _tmpSongsCollection: MutableSet<Song>
          val _tmpKey_1: Long
  ...
  private suspend fun __fetchRelationshipSongAsSong(_connection: SQLiteConnection, _map: MutableMap<Long, MutableSet<Song>>) {
    val __mapKeySet: Set<Long> = _map.keys
    if (__mapKeySet.isEmpty()) {
      return
    }
    ...
    try {
      ...
      while (_stmt.step()) {
        val _tmpKey: Long
        _tmpKey = _stmt.getLong(_itemKeyIndex)
        val _tmpRelation: MutableSet<Song>? = _map.get(_tmpKey)
  ...
  private suspend fun __fetchRelationshipSongAsSong_1(_connection: SQLiteConnection, _map: MutableMap<Long, MutableSet<Song>>) {
    val __mapKeySet: Set<Long> = _map.keys
    if (__mapKeySet.isEmpty()) {
      return
    }
